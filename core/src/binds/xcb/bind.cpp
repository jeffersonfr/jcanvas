...
/** \brief */
static jcanvas::Image *sg_icon = nullptr;
/** \brief */
...
static jcanvas::jkeyevent_symbol_t TranslateToNativeKeySymbol(xcb_keycode_t symbol)
{
	switch (symbol) {
    case 0x32:
    case 0x3e:
  ...
  xcb_change_property(sg_xcb_connection, XCB_PROP_MODE_REPLACE, sg_xcb_window, (*reply).atom, 4, 32, 1, &(*reply2).atom);
    ...
    // while (e = xcb_wait_for_event(sg_xcb_connection)) {
      ...
      if (id == XCB_EXPOSE) {
        ...
        jcanvas::jkeyevent_symbol_t symbol = TranslateToNativeKeySymbol(e->detail);
