...
public:
	...
	CBitmap(std::istream &stream) : m_BitmapData(0), m_BitmapSize(0) {
		Load(stream);
	}
	...
	bool Load(std::istream &stream) {
		...
		if (!stream.read((char*) &m_BitmapFileHeader, BITMAP_FILEHEADER_SIZE)) {
      return false;
    }
		...
		if (!stream.read((char*) &m_BitmapHeader, sizeof(BITMAP_HEADER))) {
      return false;
    }
		
		/* Load Color Table */
		
    stream.seekg(BITMAP_FILEHEADER_SIZE + m_BitmapHeader.HeaderSize, stream.beg);
		...
		if (!stream.read((char*) ColorTable, sizeof(BGRA) * m_BitmapHeader.ClrUsed)) {
      return false;
    }
    ...
    stream.seekg(m_BitmapFileHeader.BitsOffset, stream.beg);
		...
		if (m_BitmapHeader.Compression == 0) {
			for (unsigned int i = 0; i < GetHeight(); i++) {
				if (!stream.read((char*) Line, LineWidth)) {
          return false;
        }
			...
			while (stream) {
				if (!stream.read((char*) &Count, sizeof(uint8_t))) {
          return false;
        }

				if (!stream.read((char*) &ColorIndex, sizeof(uint8_t))) {
          return false;
        }
            ...
            if (!stream.read((char*) &rx, sizeof(char))) {
              return false;
            }

						if (!stream.read((char*) &ry, sizeof(char))) {
              return false;
            }
